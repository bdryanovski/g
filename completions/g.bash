_g() {
    ...
    case "${cmd}" in
        g)
            opts="-C -c -h -V --dry-run --no-interactive --help --version workspace stack commit add stage compare log status diff branch show push notes config stats developer completions help"
        ...
        g__diff)
            ...
            ;;
        g__help)
            opts="workspace stack commit add stage compare log status diff branch show push notes config stats developer completions help"
            if [[ ${cur} == -* || ${COMP_CWORD} -eq 2 ]] ; then
                COMPREPLY=( $(compgen -W "${opts}" -- "${cur}") )
                return 0
            fi
