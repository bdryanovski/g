//! Minimal GitHub REST API wrapper for stacked-PR operations and review comments.
//!
//! ## Tutorial overview
//!
//! This module handles all communication with GitHub's REST API.  It focuses on
//! the operations needed for the "Stacked PRs" workflow and the "public bucket"
//! of the review-comment system:
//!
...
//! - Posting review comments on a PR (the "public" bucket of the two-bucket
//!   review model; private notes live in the local SQLite DB instead).
//!
...
// ─── Review comments (the "public" bucket) ───────────────────────────────────

/// Which side of the diff a public review comment is anchored to.
///
/// Mirrors GitHub's `side` field on the "create review comment" endpoint.
#[derive(Debug, Clone, Copy, PartialEq, Eq, Hash)]
pub enum CommentSide {
    /// The pre-change side (`-` lines in the diff).  Maps to GitHub's `"LEFT"`.
    Old,
    /// The post-change side (`+` lines, or unchanged context).  Maps to
    /// GitHub's `"RIGHT"`.
    New,
}
...
/// Result of posting a review comment to GitHub — what the API gave us back.
#[derive(Debug, Clone)]
pub struct PostedComment {
    /// Comment id assigned by GitHub.
    pub id: u64,
    /// Web URL where a human can view the comment.
    pub html_url: String,
}

/// Post a line-anchored review comment on a PR.
///
/// This is the "public" bucket of the review system: the comment lands on the
/// GitHub PR and is visible to everyone with repo access.  Private counterpart
/// lives only in the local SQLite DB.
///
/// # Endpoints
///
/// Uses `POST /repos/{owner}/{repo}/pulls/{pr}/comments` (the "create review
/// comment" endpoint), which takes `path`, `line`, `side`, `commit_id`, and
/// `body`.  See <https://docs.github.com/rest/pulls/comments>.
///
/// # Errors
///
/// Returns an error if:
/// - The GitHub API returns a non-2xx status code.
/// - The response body cannot be parsed as JSON.
/// - `pr_number` does not identify an open PR.
#[allow(clippy::too_many_arguments)]
pub fn create_pr_review_comment(
    ...
    let resp = make_request(token, api_base, "POST", &endpoint).send_json(payload);

    match resp {
        Ok(response) => {
            let v: serde_json::Value = response
                .into_json()
                .context("Failed to parse create-review-comment response")?;
            Ok(PostedComment {
...
/// Resolve `(owner, repo, pr_number, head_sha)` for the current branch.
///
/// The SHA is the head commit of the PR the current branch is the head of.
/// Returns `Ok(None)` when the current branch is not the head of any open PR
/// — the caller can then prompt the user to publish a PR first or save a
/// private note instead.
pub fn detect_pr_for_current_branch(
    token: &str,
    api_base: &str,
    ...
    let head_sha = gitcmd::git_output(&["rev-parse", "HEAD"]).context("rev-parse HEAD")?;
    Ok(Some((owner, repo, pr.number, head_sha)))
}
