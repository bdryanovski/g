...
impl TestRepo {
    fn new() -> Self {
        ...
        let path = std::env::temp_dir().join(format!("g-test-{}-{}", std::process::id(), counter));
...
fn test_workflow_list_shows_presets() {
    ...
    assert!(
        output.contains("github-flow"),
        "Should list github-flow preset"
    );
    assert!(
        output.contains("trunk-based"),
        "Should list trunk-based preset"
    );
}
...
fn test_workflow_info_github_flow() {
    ...
    assert!(
        lower.contains("github") || lower.contains("github-flow"),
        "Should show github-flow name, got: {}",
        output
    );
...
fn test_workflow_info_invalid() {
    ...
    assert!(
        stderr.contains("not found") || stderr.contains("No workflow"),
...
fn test_workflow_use_changes_active() {
    ...
    assert!(
        ...
        "Should successfully switch to gitflow. stdout: {}, stderr: {}",
        stdout,
        stderr
    );
...
fn test_workflow_export_to_stdout() {
    ...
    assert!(
        output.contains("main_branch"),
        "Should contain main_branch, got: {}",
        output
    );
    // types is an array, so it could be [[types]] or just have name = "feature" etc.
    assert!(
        output.contains("main_branch") || output.contains("name ="),
...
fn test_workflow_clone_creates_copy() {
    ...
    assert!(
        output.status.success(),
        "Clone should succeed. stdout: {}, stderr: {}",
        stdout,
        stderr
    );
    ...
    assert!(
        ...
        "Should be able to get info on cloned workflow"
    );
}
...
fn test_workflow_status_on_main() {
    ...
    assert!(
        ...
        "Status should produce output or run silently"
    );
}
...
fn test_workflow_start_dry_run() {
    ...
    assert!(
        stdout.to_lowercase().contains("would")
            ...
            || output.status.success(),
        "Dry run should indicate what would happen. stdout: {}, stderr: {}",
        stdout,
        stderr
    );
}
